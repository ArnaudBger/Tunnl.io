...
task("post-content", "Create a deal as a brand")
  .addParam("dealid", "ID of the deal")
  .addParam("contract", "Address of the contract")
  .addParam("posturl", "URL of the post")
  ...
  .setAction(async (taskArgs) => {
    ...
    const tx = await contract.connect(influencer).postContent(
        taskArgs.dealid,
        taskArgs.posturl,
        {gasLimit, gasPrice}
    ...
    console.log(`\n Post Content transaction has the following hash ${tx.hash} on ${network.name}`)
